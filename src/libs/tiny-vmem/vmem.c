...
#define vmem_printf(...) 0
#define ASSERT(...)      assert(__VA_ARGS__, "Vmem assertation failed")
...
void *vmem_alloc_pages(size_t size)
{
    Pmm pmm = pmm_acquire();
    void *addr = (void *)unwrap_or(pmm.malloc(size * PAGE_SIZE, false), NULL).ptr;
    pmm_release(&pmm);

    return (void *)hal_mmap_lower_to_upper((uintptr_t)addr);
}
...
void vmem_dump(Vmem *vmp)
{
    ...
    debug_info("-- VMem arena \"{}\" segments -- ", vmp->name);
    ...
    TAILQ_FOREACH(span, &vmp->segqueue, segqueue)
    {
        ...
        if (span->imported)
            debug(DEBUG_NONE, "(imported)");
    }

    debug_info("Hashtable: ");

    for (i = 0; i < ARR_SIZE(vmp->hashtable); i++)
        LIST_FOREACH(span, &vmp->hashtable[i], seglist)
        {
            debug_info("{x}: [address: {a}, size {a}]", (uintptr_t)murmur64(span->base), (uintptr_t)span->base, (uintptr_t)span->size);
        }
    debug_info("Stat:");
    debug_info("- in_use: {}", vmp->stat.in_use);
    debug_info("- free: {}", vmp->stat.free);
    debug_info("- total: {}", vmp->stat.total);
}
